 ...
 */
class SmsQueue extends ResourceActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('number, body', 'required'),
			array('sent, attempts', 'numerical', 'integerOnly'=>true),
			array('number', 'length', 'max'=>200),
			array('body', 'length', 'max'=>500),
			array('message', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, number, body, sent, attempts, message', 'safe', 'on'=>'search'),
		);
	}
